...
func (rc *RmqConsumer) Chan() <-chan mqclient.Message {
	if rc.msgChannel == nil {
		rc.once.Do(func() {
			rc.msgChannel = make(chan mqclient.Message, 256)
			rc.wg.Add(1)
			go func() {
				defer rc.wg.Done()
				for { //nolint:gosimple
...
func (rc *RmqConsumer) Seek(id mqclient.MessageID, inclusive bool) error {
...
func (rc *RmqConsumer) Ack(message mqclient.Message) {
}
