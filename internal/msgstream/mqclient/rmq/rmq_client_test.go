...
import (
	...
	rocksmq1 "github.com/milvus-io/milvus/internal/msgstream/mqimpl/rocksmq/server/rocksmq"
	"os"
	"testing"
	"time"
...
func TestRmqClient_CreateProducer(t *testing.T) {
	...
	proOpts := mqclient.ProducerOptions{Topic: topic}
	...
	invalidOpts := mqclient.ProducerOptions{Topic: ""}
...
func TestRmqClient_Subscribe(t *testing.T) {
	...
	subName := "subName"
	consumerOpts := mqclient.ConsumerOptions{
		Topic:                       "",
...
func TestRmqClient_BytesToMsgID(t *testing.T) {
	...
	binary := pulsar2.SerializePulsarMsgID(mid)
